...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 JumpServer 复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://github.com/jumpserver/docker#recommended-system-requirements)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r jumpserver
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r jumpserver -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-jumpserver
```
...
```
git clone --depth=1 https://github.com/Websoft9/docker-jumpserver
cd docker-jumpserver
sudo docker-compose -f docker-compose.yml --env-file .env up -d
```

### 常见问题

#### 安装 JumpServer 前需要更改密码吗？
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| jumpserver-server | 9002 | 浏览器访问 JumpServer | Y |
| phpmyadmin | 9090 | 数据库可视化管理工具 | Y |
## 文档

[JumpServer 管理员手册](https://support.websoft9.com/docs/jumpserver)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [JumpServer 企业级支持版](https://apps.websoft9.com/jumpserver) 
